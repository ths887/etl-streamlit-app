...
""", unsafe_allow_html=True )
    ...
    st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
    st.caption(f"Page {page + 1} of {n_pages}  ·  {mapped_count} field(s) mapped  ·  {total - mapped_count} skipped")

    n1, n2, n3, n4 = st.columns([2, 2, 2, 4])
    with n1:
    ...
    with n3:
        if st.button("Next ➡", disabled=(page >= n_pages - 1)):
            ...
            st.rerun()
    with n4:
        if st.button("Save & Next: Attribute Grouping →", type="primary", use_container_width=True):
            if mapped_count == 0:
                st.warning("Please map at least one source field before continuing.")
            else:
